...
@Component({
  selector: 'app-old-forests',
  templateUrl: './old-forests.component.html',
  styleUrls: ['./old-forests.component.css']
})
export class OldForestsComponent implements OnInit {
  forestID: string;
  ...
  withdrawAllAndHarvest() {
    const func = this.contract.getOldForest(this.forestID).methods.exit();
    this.wallet.sendTx(func, () => { }, () => {
      this.loadData();
    }, () => { });
  }
